...
WORKSHEET_NAME = get("WORKSHEET_NAME", "reserv")  # Default to "reserve" if not set
# print("Using SHEET_ID:", SHEET_ID)
# print("Using WORKSHEET_NAME:", WORKSHEET_NAME)
...
if st.button("Validate"):
    if not code_input or code_input.strip() == "":
        st.warning("⚠️ Please enter a valid coupon code.")
    else:
        log_event("CouponCodeValidationAttempt", email=user_email, details=f"Code: {code_input}")
    ...
    if record['Status'].lower() == "expired":
        ...
        with st.form("expire_form"):
            ...
            if submit:
                ...
                # log_event("CouponCodeExpired", email=user_email, details=f"Code: {code_input}, Expiry Date: {expiry_date}, Comments: {comments}")
